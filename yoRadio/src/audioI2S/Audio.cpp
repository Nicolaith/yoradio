 ...
 *
 *  Version 3.0.12u
 *  Updated on: Sep 30.2024, Oct 08.2024 (Maleksm)
...
//    else                            {m_f_ssl = false; hostwoext_begin = 7; port = 80;}
    ...
    //    log_i("connecttohost(): %s", rqh);
        ...
        if(endsWith(h_host, ".m3u8")) { m_expectedPlsFmt = FORMAT_M3U8; if(audio_lasthost) audio_lasthost(host); }
...
bool Audio::connecttoFS(fs::FS& fs, const char* path, int32_t fileStartPos) {
    ...
    if(endsWith(path, ".mp3")) { m_codec = CODEC_MP3; audio_info("format is mp3"); }
//    if(endsWith(path, ".m4a")) { m_codec = CODEC_M4A; audio_info("format is aac"); }
//    if(endsWith(path, ".aac")) { m_codec = CODEC_AAC; audio_info("format is aac"); }
//    if(endsWith(path, ".wav")) { m_codec = CODEC_WAV; audio_info("format is wav"); }
    if(endsWith(path, ".flac")) { m_codec = CODEC_FLAC; audio_info("format is flac"); }
    if(endsWith(path, ".opus")) { m_codec = CODEC_OPUS; audio_info("format is opus"); }
    if(endsWith(path, ".ogg")) { m_codec = CODEC_OGG; audio_info("format is ogg"); }
//    if(endsWith(path, ".oga")) { m_codec = CODEC_OGG; audio_info("format is ogg"); }
...
void Audio::processLocalFile() {
    ...
    if(!m_f_stream) {
        if(m_codec == CODEC_OGG) { 					// log_i("determine correct codec here");
            ...
            else if(codec == CODEC_OPUS) { m_codec = CODEC_OPUS; audio_info("format is opus"); initializeDecoder(); return; }
            else if(codec == CODEC_VORBIS) { m_codec = CODEC_VORBIS; audio_info("format is vorbis"); initializeDecoder(); return; }
