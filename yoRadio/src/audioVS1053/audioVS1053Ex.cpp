 ...
 *  Updated on: Oct 10,2024 (Maleksm)
 *      Author: Wolle (Oct 23.2023)
 */
...
//  if(!_vuInitalized) return;
  ...
  uint8_t left = reg & 0x00FF; 				// LSB left channel
  uint8_t right = reg >> 8; 				// MSB left channel
  vuLeft = map(left, 88, 95, 0, 255);			//  vuLeft = map(left, 85, 92, 0, 255);
  vuRight = map(right, 88, 95, 0, 255);			//  vuRight = map(right, 85, 92, 0, 255);
//  vuLeft = map((uint8_t)(reg & 0x00FF), 88, 94, 0, 255);
//  vuRight = map((uint8_t)(reg >> 8), 88, 94, 0, 255);
}
//###################################################################
...
void Audio::processWebStream() {
    ...
    if(availableBytes) {
        ...
        if(m_codec == CODEC_OGG){ 					// log_i("determine correct codec here");
            uint8_t codec = determineOggCodec(InBuff.getReadPtr(), maxFrameSize);
            if(codec == CODEC_FLAC) {m_codec = CODEC_FLAC; AUDIO_INFO("format is flac"); return;}
            if(codec == CODEC_VORBIS) {m_codec = CODEC_VORBIS; AUDIO_INFO("format is vorbis"); return;}
            if(codec == CODEC_OPUS) {m_codec = CODEC_OPUS; AUDIO_INFO("format is opus"); AUDIO_ERROR("can't play OPUS yet"); return;}
            stopSong();
            return;
        }
    }
...
void Audio::processWebFile(){
 ...
 //   static uint32_t byteCounter;                                // count received data
    ...
    if(m_codec == CODEC_OGG) { AUDIO_INFO("format is ogg"); 			// log_i("determine correct codec here");
        uint8_t codec = determineOggCodec(InBuff.getReadPtr(), maxFrameSize);
        if(codec == CODEC_VORBIS) {m_codec = CODEC_VORBIS; AUDIO_INFO("format is vorbis"); return;}
        else if(codec == CODEC_FLAC)   {m_codec = CODEC_FLAC; AUDIO_INFO("format is flac"); return;}
        else if(codec == CODEC_OPUS)  {m_codec = CODEC_OPUS; AUDIO_INFO("format is opus");
...
//    bool res = false;
    ...
    if(endsWith(path, ".mp3")) { m_codec = CODEC_MP3; m_f_running = true; audio_info("format is mp3"); }
//    if(endsWith(path, ".m4a")) { m_codec = CODEC_M4A; m_f_running = true; audio_info("format is aac"); }
//    if(endsWith(path, ".aac")) { m_codec = CODEC_AAC; m_f_running = true; audio_info("format is aac"); }
//    if(endsWith(path, ".wav")) { m_codec = CODEC_WAV; m_f_running = true; audio_info("format is wav"); }
    if(endsWith(path, ".flac")) { m_codec = CODEC_FLAC; m_f_running = true; audio_info("format is flac"); }
    if(endsWith(path, ".opus")) { m_codec = CODEC_OPUS; m_f_running = true; audio_info("format is opus"); }
    if(endsWith(path, ".ogg")) { m_codec = CODEC_OGG; m_f_running = true; audio_info("format is ogg"); }
//    if(endsWith(path, ".oga")) { m_codec = CODEC_OGG; m_f_running = true; audio_info("format is ogg"); }

		// m_codec is by default CODEC_NONE
    if(m_codec == CODEC_NONE) {AUDIO_INFO("The %s format is not supported", path + dotPos); m_f_running = false; /*res = false;*/ goto exit;}   // guard
...
//    res = m_f_running;
//    if(!res) audiofile.close();
    if(!m_f_running) audiofile.close();
